...
% \usepackage[show]{chato-notes} % will hide all notes
% N.B. must have a "chato-notes.sty" file in the same directory of "main.tex"
\author{}
\title{}
\begin{document}

\note{empty} % Multiple paragraphs to check fonts
\note[author=Matteo,to=Rionda]{for and from}
\note[author=Matteo,to=]{from only (empty for)}
\note[author=Matteo]{from only}
\note[to=Rionda,author=]{for only (empty from)}
\note[to=Rionda]{for only}
